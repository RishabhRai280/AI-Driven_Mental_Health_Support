...
export default function Header({
  ...
  useEffect(() => {
    const loadThemeAndAvatar = () => {
      ...
      let savedTheme = localStorage.getItem("sm_theme") as
        | "light"
        | "dark"
        | null;
      if (!savedTheme) {
      ...
      if (saved && avatarData[saved]) {
        ...
        setAvatarImagePath(avatarData[saved].imagePath);
      } else {
        setUserAvatar("RC");
        setAvatarBg("var(--color-primary)");
        setAvatarImagePath("/Avatars/users-avatar-svgrepo-com.svg");
      }
    };
  ...
  useEffect(() => {
    async function loadStreak() {
      ...
      if (!token) return;
      try {
        const data = await api.get<{ streak: number }>("/api/wellness/streak");
          ...
          {showNotifications && (
            ...
            >
              ...
              >
                ...
                >
                  <strong>Sparky Companion:</strong> Let&apos;s complete a short
                  breathing exercise today!
                </div>
                <div
                  style={{ fontSize: "13px", color: "var(--text-secondary)" }}
                >
                  Daily journal reminder: Don&apos;t forget to check in on your
                  feelings.
                </div>
              </div>
            </div>
